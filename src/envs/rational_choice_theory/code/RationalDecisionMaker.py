...
class RationalDecisionMaker(GeneralAgent):
    ...
    async def evaluate_choices(self, event: Event) -> List[Event]:
        ...
        candidate_ids = await self.env.get_agent_data_by_type("UtilityEvaluator", "id")
        ...
        """
        observation = f"Decision options: {decision_options}, Personal goals: {personal_goals}, Resource status: {resource_status}, Candidate IDs of UtilityEvaluator: {candidate_ids}"
    ...
    async def make_decision(self, event: Event) -> List[Event]:
        ...
        candidate_ids = await self.env.get_agent_data_by_type("SocialNetworkInteractor", "id")
        ...
        - Canddiate IDs of ocialNetworkInteractor agent: {candidate_ids}
